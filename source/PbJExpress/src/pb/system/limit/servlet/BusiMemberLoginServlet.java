 ...
 */
public class BusiMemberLoginServlet extends AbstractServlet
{
    /**
     * 方法（受保护）<br>
            ...
            throws ServletException, IOException
    {
        ...
        //检查用户是否已经登录
        boolean isLogin=((BusiMemberAction)this.dataAction).isLogin(request);

        //如果用户未登录，执行登录处理
        if(!isLogin)
        {
            isLogin=((BusiMemberAction)this.dataAction).login(request);
        }
